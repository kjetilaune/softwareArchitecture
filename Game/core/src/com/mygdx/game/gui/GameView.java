...
public class GameView extends AbstractView implements Screen, Observer{
    ...
    public void render(float delta) {
        ...
        for (Player p : players) {
            ...
            font.draw(batch, Integer.toString(p.getVehicle().getHealth()), p.getVehicle().getPosition().x + p.getVehicle().getRelativeWidth()/2, p.getVehicle().getPosition().y + p.getVehicle().getRelativeHeight() + 10);
        }
